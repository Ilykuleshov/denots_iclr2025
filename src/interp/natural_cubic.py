...
class NaturalCubicSpline(BaseInterpolator):
    ...
    def fit(self, t: Tensor, x: Tensor):
        ...
        Args:
        ----
            t (Tensor): tensor of shape (B, L) or (B, L, C) for channelwise fitting.
            x (Tensor): tensor of shape (B, L, C).
        ...
        match t.ndim:
            case 3:
                fit_vmapped = torch.vmap(fit_vmapped, -1, -1)
                eval_vmapped = torch.vmap(eval_vmapped, (-1, -1, None), -1)
            case 2:
                fit_vmapped = torch.vmap(fit_vmapped, (None, -1), -1)
            case _:
                raise ValueError(t)

        self.coeffs: Tensor = fit_vmapped(t, x)  # (B, 4, L, C)
